...
function Transactions({ transactions = [], onDelete, fm, selectedMonth, setSelectedMonth }) {
  ...
  return (
    <div className="p-8 2xl:p-12 pb-[100px] 2xl:pb-[140px]">
      {/* Header & Search */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 2xl:gap-10 mb-8 2xl:mb-12">
        <div className="flex flex-wrap gap-2">
          {['all', 'expense', 'income'].map((f) => (
            <button 
              key={f}
              onClick={() => setFilter(f)} 
              className={`px-6 py-2.5 2xl:px-8 2xl:py-3.5 font-black rounded-xl text-xs 2xl:text-sm uppercase tracking-widest cursor-pointer transition-colors duration-200 ${filter === f ? (f === 'expense' ? 'bg-red-400 text-slate-950 shadow-[0_0_20px_rgba(248,113,113,0.3)]' : 'bg-emerald-400 text-slate-950 shadow-[0_0_20px_rgba(74,222,128,0.3)]') : 'bg-slate-900/50 border border-slate-700/40 text-slate-400 hover:text-slate-100 hover:bg-slate-800/60'}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full md:w-auto">
          <input 
            ...
            className="h-11 2xl:h-14 bg-slate-950/55 border border-slate-700/40 rounded-xl px-4 2xl:px-6 2xl:text-lg text-slate-100 font-bold outline-none focus:border-emerald-400/70 transition-colors [color-scheme:dark]"
          />
          <div className="relative w-full md:w-64 2xl:w-80 group transition-colors duration-300">
            ...
            <input 
              ...
              className="w-full h-11 2xl:h-14 bg-slate-950/55 border border-slate-700/40 rounded-xl pl-12 2xl:pl-14 pr-4 2xl:pr-6 text-slate-100 placeholder:text-slate-500 outline-none backdrop-blur transition-colors duration-200 focus:border-emerald-400/70 focus:ring-2 focus:ring-emerald-400/10 text-sm 2xl:text-base shadow-inner"
            />
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-950/35 border-b border-slate-700/30">
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500">Date</th>
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500">Transaction</th>
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500">Category</th>
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500">Method</th>
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-right text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500">Amount</th>
              <th className="px-8 2xl:px-12 py-5 2xl:py-6 text-center text-[10px] 2xl:text-sm font-black uppercase tracking-[0.2em] text-slate-500"></th>
            </tr>
          </thead>
          <motion.tbody 
            variants={container}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((t_data) => (
                <motion.tr key={t_data.id} variants={item} className="hover:bg-slate-800/30 transition-colors duration-200 group border-b border-slate-700/10 last:border-0">
                  <td className="px-8 2xl:px-12 py-3.5 2xl:py-5 text-xs 2xl:text-base font-bold text-slate-500 tracking-tight">
                    {formatDate(t_data.date)}
                  </td>
                  <td className="px-8 2xl:px-12 py-3.5 2xl:py-5 group-hover:text-emerald-400 transition-colors tracking-tight">
                    <div>
                      <p className="font-bold text-slate-100 group-hover:text-emerald-400 transition-colors 2xl:text-lg">
                        {t_data.title}
                      </p>
                      {t_data.notes && (
                  ...
                  <td className="px-8 2xl:px-12 py-3.5 2xl:py-5">
                    <span className="px-2.5 py-0.5 2xl:px-4 2xl:py-1.5 bg-slate-800/50 text-slate-400 rounded-lg text-[9px] 2xl:text-xs uppercase font-black tracking-widest border border-white/5">{t_data.category}</span>
                  </td>
                  <td className="px-8 2xl:px-12 py-3.5 2xl:py-5">
                    <div className="flex items-center gap-2 text-[10px] 2xl:text-xs font-black uppercase tracking-widest text-slate-500">
                      <span className="material-symbols-outlined text-[16px] 2xl:text-[20px]">wallet</span>
                      {t_data.method}
                    </div>
                  </td>
                  <td className={`px-8 2xl:px-12 py-3.5 2xl:py-5 text-right font-black tracking-tighter text-lg 2xl:text-2xl transition-colors duration-200 ${t_data.type === 'income' ? 'text-emerald-400' : 'text-slate-100'}`}>
                    {t_data.type === 'income' ? '+' : '-'} {fm(t_data.amount)}
                  </td>
                  <td className="px-8 2xl:px-12 py-3.5 2xl:py-5 text-center">
                    <button onClick={() => onDelete && onDelete(t_data.id)} className="text-red-400/50 hover:text-red-400 hover:bg-red-400/10 p-2 2xl:p-3 rounded-xl transition-all duration-200">
                      <span className="material-symbols-outlined font-bold text-[20px] 2xl:text-2xl">delete</span>
                    </button>
                  </td>
                </motion.tr>
              ))
